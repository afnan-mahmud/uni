...
export function getAcademicConfig(module: string): ModuleConfig | undefined {
  switch (module) {
    // ---------- Institution & Structure ----------
    case "institution-structure":
      return {
    ...
    case "section-management":
      return isFaculty() && !isAdminView()
        ? {
            title: "My Classes",
            description: "Your assigned course sections for the current semester.",
            primaryButton: "View Class Roster",
            kpis: [
              { label: "Assigned Sections", value: String(F.assignedCourses.length), color: "border-l-indigo-500" },
              { label: "Total Students", value: String(F.assignedCourses.reduce((sum, c) => sum + c.enrolled, 0)), color: "border-l-emerald-500" },
            ],
            columns: ["Course", "Section", "Credits", "Enrolled", "Capacity", "Status"],
            data: F.assignedCourses.map((c) => [c.code, c.section, String(c.credits), String(c.enrolled), String(c.capacity), c.enrolled >= c.capacity ? "Full" : "Open"]),
          }
        : {
            ...
            data: [
              ["CSE301", "A", "Dr. Kamal Uddin", "40", "38", "Open"],
              ["CSE301", "B", "Dr. Salim", "40", "40", "Full"],
    ...
    case "class-scheduling":
      return isFaculty() && !isAdminView()
        ? {
            ...
            primaryButton: "Export Schedule",
            kpis: [
              { label: "Weekly Hours", value: "18", color: "border-l-indigo-500" },
              { label: "Courses", value: String(F.assignedCourses.length), color: "border-l-emerald-500" },
            ],
            columns: ["Day", "Time", "Course", "Section", "Room"],
            data: [
              ["Sunday", "09:00 AM - 10:30 AM", "CSE301", "A", "Room 401"],
              ["Sunday", "10:45 AM - 12:15 PM", "CSE302", "A", "Lab 1"],
              ["Sunday", "01:15 PM - 02:45 PM", "CSE401", "B", "Room 402"],
              ["Tuesday", "09:00 AM - 10:30 AM", "CSE301", "A", "Room 401"],
              ["Tuesday", "10:45 AM - 12:15 PM", "CSE302", "A", "Lab 1"],
              ["Wednesday", "01:15 PM - 02:45 PM", "CSE401", "B", "Room 402"],
              ["Thursday", "09:00 AM - 10:30 AM", "CSE301", "A", "Room 401"],
            ],
          }
        : {
    ...
    case "advising":
      return isFaculty() && !isAdminView()
        ? {
            title: "My Advisee Students",
            description: "Students assigned under your academic advisement.",
            primaryButton: "Approve Registrations",
            kpis: [
              { label: "Advisees", value: String(F.advisees.length), color: "border-l-indigo-500" },
              { label: "Pending Approvals", value: "1", color: "border-l-amber-500" },
            ],
            columns: ["Student ID", "Name", "Program", "Semester", "Registration Status"],
            data: F.advisees.map((a) => [a.studentId, a.name, a.program, a.semester, a.studentId === "191-15-12346" ? "Pending" : "Approved"]),
          }
        : {
    ...
    case "requests":
      return isFaculty() && !isAdminView()
        ? {
            ...
            description: "Course withdrawal, grade review, and program transfer requests for your advisees.",
            primaryButton: "Review Requests",
            kpis: [
              { label: "Pending", value: "2", color: "border-l-amber-500" },
              { label: "Processed Today", value: "1", color: "border-l-emerald-500" },
            ],
            columns: ["Request ID", "Student", "Type", "Submission Date", "Status"],
            data: [
              ["REQ-001", "Afnan Mahmud", "Grade Review (CSE301)", "2026-08-10", "Pending HOD"],
              ["REQ-004", "Sabrina Akter", "Course Withdrawal", "2026-08-11", "Pending Advisor"],
            ],
          }
        : {
            title: "Academic Requests",
    ...
    case "attendance":
      return isFaculty() && !isAdminView()
        ? {
            ...
            description: "Mark and review attendance for your assigned classes.",
            primaryButton: "Generate Report",
            kpis: [
              { label: "Classes Today", value: "3", color: "border-l-indigo-500" },
              { label: "Avg Attendance", value: "91%", color: "border-l-emerald-500" },
            ],
            columns: ["Course", "Section", "Date", "Total Students", "Present", "Absent"],
            data: [
              ["CSE301", "A", "2026-08-11", "38", "36", "2"],
              ["CSE302", "A", "2026-08-11", "38", "37", "1"],
              ["CSE401", "B", "2026-08-11", "35", "33", "2"],
            ],
          }
        : {
    ...
    case "assessment":
      return isFaculty() && !isAdminView()
        ? {
            title: "Assessments Setup",
            description: "Configure assessment components for your courses.",
            primaryButton: "Save Changes",
            kpis: [
              { label: "Active Assessments", value: "9", color: "border-l-indigo-500" },
              { label: "Pending Setup", value: "2", color: "border-l-amber-500" },
            ],
            columns: ["Course", "Section", "Component", "Weightage", "Status"],
            data: [
              ["CSE301", "A", "Midterm", "30%", "Completed"],
              ["CSE301", "A", "Final", "40%", "Pending"],
              ["CSE301", "A", "Quizzes (Best 3)", "20%", "Ongoing"],
              ["CSE401", "B", "Midterm", "30%", "Completed"],
              ["CSE401", "B", "Final", "40%", "Pending"],
            ],
          }
        : {
    ...
    case "examination":
      return isFaculty() && !isAdminView()
        ? {
            title: "My Exam Duties",
            description: "Your assigned invigilation and exam duties.",
            primaryButton: "View Seat Plan",
            kpis: [
              { label: "Invigilation Duties", value: "4", color: "border-l-indigo-500" },
              { label: "This Week", value: "2", color: "border-l-amber-500" },
            ],
            columns: ["Exam", "Date", "Time", "Room", "Role"],
            data: [
              ["CSE301 Final", "2026-08-25", "10:00 AM", "Room 401", "Invigilator"],
              ["CSE401 Final", "2026-08-27", "02:00 PM", "Room 402", "Invigilator"],
              ["Makeup Exam", "2026-08-30", "10:00 AM", "Hall A", "Invigilator"],
            ],
          }
        : {
